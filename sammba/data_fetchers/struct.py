...
def fetch_lemur_mircen_2019_t2(
    subjects=[0], data_dir=None, url=None, resume=True, verbose=1
):
    ...
    anat_basenames = [
        "{0}_anat_{0}_T2w.nii.gz".format(subject_id) for subject_id in subject_ids
    ]
    anat_files = [
    ...
    json_basenames = [
        "{0}_anat_{0}_T2w.json".format(subject_id) for subject_id in subject_ids
    ]
    json_files = [
    ...
    anat = []
    json = []
    for anat_u, anat_f, json_u, json_f in zip(
        anat_urls, anat_files, json_urls, json_files
    ):
        a, j = _fetch_files(
            data_dir,
            [(anat_f, anat_u, {"move": anat_f}), (json_f, json_u, {"move": json_f})],
            verbose=verbose,
        )
        json.append(j)
        anat.append(a)
    ...
    phenotypic = phenotypic[
        [np.where(phenotypic["animal_id"] == '"' + i + '"')[0][0] for i in subject_ids]
    ]
